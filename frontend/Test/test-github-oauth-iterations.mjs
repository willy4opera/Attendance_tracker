...
class GitHubOAuthIterativeTester {
  constructor() {
  ...
  async runIterations() {
    console.log(chalk.bold.blue('🚀 GitHub OAuth Iterative Test - Multiple Login/Logout Cycles\n'));
    console.log(chalk.gray('This test will help you test multiple GitHub OAuth login cycles'));
    ...
    while (true) {
      ...
      try {
        ...
        console.log(chalk.yellow('📍 Step 1: Generating GitHub OAuth URL...'));
        ...
        console.log(chalk.white('2. Complete GitHub authentication'));
  ...
  async getOAuthUrl() {
    try {
      ...
      const response = await axios.get(`${this.baseURL}/auth/oauth/github/url`, {
        params: { state }
      });
      
      return response.data.data?.url || response.data.url;
    } catch (error) {
      throw new Error(`Failed to get GitHub OAuth URL: ${error.message}`);
    }
  }

  async exchangeCodeForToken(code, state) {
    try {
      const response = await axios.post(`${this.baseURL}/auth/oauth/github`, {
        code,
      ...
      if (response.data.token) {
        ...
        if (response.data.data && response.data.data.user) {
          ...
          console.log(chalk.gray(`  - GitHub ID: ${user.githubId || 'N/A'}`));
        }
      } else {
...
// Show intro
console.log(chalk.bold.cyan('GitHub OAuth Multiple Iteration Test\n'));
console.log(chalk.white('This test will help you:'));
console.log(chalk.gray('• Test multiple GitHub OAuth login cycles'));
...
console.log(chalk.yellow('\n⚠️  Note: Make sure GitHub OAuth is properly configured in your backend'));
console.log(chalk.gray('   - Client ID and Client Secret should be set'));
console.log(chalk.gray('   - Redirect URI should be configured in GitHub App'));

// Start the test
async function main() {
  await question(chalk.cyan('\nPress ENTER to start testing...'));
  const tester = new GitHubOAuthIterativeTester();
