      ...
      try {
        ...
        switch (provider?.toLowerCase()) {
          case 'github':
            ...
            break
          case 'linkedin':
            console.log('[OAuthCallback] LinkedIn auth - code:', code?.substring(0, 20) + '...', 'state:', state?.substring(0, 20) + '...')
            result = await authService.handleLinkedInAuth(code, state)
            break
          case 'facebook':
        ...
        if (provider?.toLowerCase() === 'linkedin') {
          ...
          if (error.isRecoverable || error.message?.includes('already connected') || error.message?.includes('already authorized')) {
          ...
          // Check for LinkedIn rate limit / profile block
          if (error.isLinkedInRateLimit || error.message?.includes('temporarily blocked')) {
            toastWarning('LinkedIn has temporarily blocked access. This is normal after multiple logins. Please wait a few minutes before trying again.');
            setTimeout(() => {
              navigate('/login')
            }, 3000)
            return
